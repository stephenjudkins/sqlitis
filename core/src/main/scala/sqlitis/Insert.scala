...
object Insert {
  ...
  def value[T[_ <: Ctx]](_row: T[Ctx.Inserted]):Inserter[T, Long] = new Inserter[T, Long] {
    def row = _row
  }
  ...
  object BuildInsert {
    implicit def idInstance[T[_ <: Ctx], R, IH <: HList, SH <: HList, FH <: HList](
      implicit
  ...
  object InsertHList {
    ...
    implicit def insertHNil: InsertHList[HNil, HNil, HNil] = new InsertHList[HNil, HNil, HNil] {
