...
object Parser {
  ...
  private val functionCall: Parser[Expression[Unit]] = (identifier ~ parens(sepBy(expr, comma))).map {
    case (n, args) =>
      FunctionCall(n.name, args)
  }

  private val singleExpr: Parser[Expression[Unit]] = token(
    not | functionCall | identifier.widen[Expression[Unit]] | literal.widen[Expression[Unit]] | parens(expr)
  )
  ...
  implicit val expr: Parser[Expression[Unit]] = opsByPrecedence
    .foldLeft(singleExpr) {
      case (e, level) =>
  ...
  implicit val select: Parser[Select[Unit]] = (
    ...
    case (((((isDistinct, fields), from), where), orderBy), limit) =>
      Select(
  ...
  implicit val insert: Parser[Insert[Unit]] = (
    kw("INSERT") ~> kw("INTO") ~> token(idToken) ~ parens(sepBy(token(idToken), comma)) ~ (s ~> kw("VALUES") ~> parens(
      sepBy(expr, comma)
    ))
