...
object Generator {
  ...
  private implicit class SqlHelper(val sc: StringContext) extends AnyVal {
    def sql[A](args: Out[A]*):Out[A] = {
      def impl(parts: List[String], args: List[Out[A]], accumA: List[A], accumS: List[String]):Out[A] = parts match {
        case s :: ss => args match {
  ...
  implicit object GenExpr extends Generator[Expression] {
    ...
    def generate[A](expr: Expression[A]):Out[A] = expr match {
      case Identifier(t, c) => {
