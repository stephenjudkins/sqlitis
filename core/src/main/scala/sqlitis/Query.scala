...
object Query {
  ...
  case class Ref[+X, A](expr: Expression[X]) {
    def ===[Y](other: Ref[Y, A])(implicit c: Ref[X, A] <:< Ref[Y, A]): Ref[Y, Boolean] =
      Ref[Y, Boolean](Equals[Y](c(this).expr, other.expr))
  }
  ...
  def apply[T[_ <: Ctx]](implicit table: Table[T], querify: Querify[T]): Q[Unit, T[Queried[Unit]]] =
    new Q[Unit, T[Queried[Unit]]] {
  ...
  object Querify {
    def apply[T[_ <: Ctx]](tableAlias: String, t: T[Schema])(implicit q: Querify[T]): T[Queried[Unit]] =
      q(tableAlias, t)
    ...
    implicit def assignAliasToTablesHCons[A, H0 <: HList, H1 <: HList](implicit
        tail: AssignAliasToTables[H0, H1]
  ...
  object ResultExtractor {
    ...
    implicit def hnilInstance[X]: ResultExtractor[HNil, HNil, X] =
      new ResultExtractor[HNil, HNil, X] {
        def apply(accum: List[Field[X]], a: HNil): List[Field[X]] = accum
      }

    implicit def hConsInstance[I, O, HI <: HList, HO <: HList, X](implicit
        h: ResultExtractor[I, O, X],
    ...
    implicit def refInstance[A, X]: ResultExtractor[Ref[X, A], A, X] =
      new ResultExtractor[Ref[X, A], A, X] {
        def apply(accum: List[Field[X]], a: Ref[X, A]): List[Field[X]] = ExpressionField(a.expr, None) :: accum
      }

    implicit def tableInstance[T[_ <: Ctx], HI <: HList, HO <: HList, X](implicit
        from: Generic.Aux[T[Queried[X]], HI],
        to: Generic.Aux[T[Concrete], HO],
        gen: ResultExtractor[HI, HO, X]
    ): ResultExtractor[T[Queried[X]], T[Concrete], X] =
      new ResultExtractor[T[Queried[X]], T[Concrete], X] {
        def apply(accum: List[Field[X]], a: T[Queried[X]]): List[Field[X]] = gen(accum, from.to(a))
      }

    implicit def tupleInstance[I, HI <: HList, HO <: HList, O, X](implicit
        from: Generic.Aux[I, HI],
        gen: ResultExtractor[HI, HO, X],
        to: Tupler.Aux[HO, O],
        isTuple: Tupler.Aux[HI, I]
    ): ResultExtractor[I, O, X] =
      new ResultExtractor[I, O, X] {
  ...
  trait Q[X, A] { self =>
    def run[O](implicit resultExtractor: ResultExtractor[A, O, X]): SelectResult[X, O] = {
      ...
      val select = Select(
        ...
        from = queryState.relations.toList.map {
          case (alias, t) => TableName(t, if (alias == t) None else Some(alias))
        },
    ...
    def withFilter(f: A => Ref[X, Boolean]): Q[X, A] =
      new Q[X, A] {
        def apply(s: QueryState[X]): (A, QueryState[X]) = {
          val (a, s2)    = self(s)
